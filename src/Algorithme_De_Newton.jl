...
function Algorithme_De_Newton(f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    while flag == -1
        ...
        if norm(gradf(xmin)) <= max(Tol_rel * norm(g0), Tol_abs)
            flag = 0
        elseif norm(xmin - xk) <= epsilon * max(Tol_rel * norm(xk), Tol_abs)
            flag = 1
        elseif abs(f(xmin) - f(xk))  <= epsilon * max(Tol_rel * abs(f(xk)), Tol_abs)
            flag = 2
