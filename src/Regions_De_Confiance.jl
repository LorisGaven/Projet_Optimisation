...
function Regions_De_Confiance(algo,f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    while flag == -1
        if algo == "cauchy"
            ...
            s = Gradient_Conjugue_Tronque(gradf(xmin), hessf(xmin), [delta, max_iter, Tol_rel])
        end

        m = f(xmin) + transpose(gradf(xmin))*s + (1/2)*transpose(s)*hessf(xmin)*s
        ρ = (f(xmin) - f(xmin + s)) / (f(xmin) - m)
        ...
        if norm(gradf(xmin)) <= max(Tol_rel * norm(g0), Tol_abs) && changer
            flag = 0
        elseif norm(xmin - xk) <= epsilon * max(Tol_rel * norm(xk), Tol_abs) && changer
            flag = 1
        elseif abs(f(xmin) - f(xk))  <= epsilon * max(Tol_rel * abs(f(xk)), Tol_abs) && changer
            flag = 2
    ...
    return xmin, f(xmin), flag, nb_iters
end
